...
1. **Find planned topics**: Scan all `50_Topics/*/_index.md` files for those with `status: planned`. If `$ARGUMENTS` is provided, only research those specific topic names.
...
2. **Research topics using parallel agents**: Launch one Agent per topic in parallel. Each agent handles a single topic independently:
   ...
   > Research the topic `<topic-name>`. Read `50_Topics/<topic>/_index.md` to understand the key questions and scope, then:
   >
   ...
   > - **Create research notes** in `50_Topics/<topic>/notes/`: One note per major theme or key question cluster. Use the note template format with proper frontmatter. Include detailed findings, analysis, and data. Add `[[wikilinks]]` to related topics and references. Filenames: descriptive, lowercase, hyphen-separated.
   > - **Create reference files** in `50_Topics/<topic>/references/`: One file per source using the reference template format. Include source URL, authors, summary, key points. Add relevance notes linking back to the research questions.
   ...
   > - **Update 90_Maps/**: If this topic connects multiple existing topics, add or update a Map of Content in `90_Maps/`.
   >
...
3. **Cross-reference**: After all agents complete, check if any of the researched topics are related to each other or to existing topics in `50_Topics/`. Add `[[wikilinks]]` in both directions where relevant.
