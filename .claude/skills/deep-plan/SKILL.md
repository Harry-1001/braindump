...
2. **Check for duplicates**: For each topic word, check if a topic already exists under `50_Topics/` that covers the same subject. If so, skip it and note that it already exists.

3. **Create research plans**: For each new topic:
   - Create the directory structure: `50_Topics/<topic-name>/`, `50_Topics/<topic-name>/notes/`, `50_Topics/<topic-name>/references/`
   - Add `.gitkeep` files in `notes/` and `references/`
   - Create `50_Topics/<topic-name>/_index.md` using the topic template format with:
   ...
   - Check existing topics in `50_Topics/` and add relevant `[[wikilinks]]` in the Related Topics section
